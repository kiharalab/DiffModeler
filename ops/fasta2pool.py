...
from ops.pdb_utils import count_atom_line, count_residues
from ops.fasta_searchdb import download_pdb, get_afdb_pdb_url
...
def fasta2pool(params, save_path):
    ...
    for chain_name_list in chain_dict:
        ...
        final_pdb_path = os.path.join(
            final_pdb_dir, chain_name_list[:max_file_name_limit] + ".pdb"
        )
        ...
        command_line = (
            "cd %s; python %s --email %s --program fasta "
            "--stype protein --database pdb,afdb "
            "--sequence %s"
            % (current_chain_dir, search_script, params["email"], input_fasta_path)
        )
    ...
    for chain_name_list in chain_dict:
        ...
        if len(listfiles_acc) == 0:
            print(
                "fail to find search results for chain %s *.accs.txt from EBI-Search!"
                % chain_name_list
            )
            exit()
        ...
        for candidate_index, candidate in enumerate(candidate_list):
            ...
            expected_seq_length = (
                len(chain_dict[chain_name_list.replace("-", ",")])
                * params["search"]["length_ratio"]
            )
            actual_structure_length = count_residues(final_pdb_path)
            max_allow_length = (
                len(chain_dict[chain_name_list.replace("-", ",")])
                * params["search"]["max_length_ratio"]
            )
            if (
                ...
                print(
                    "structure length %d is not in the range of %d and %d"
                    % (actual_structure_length, expected_seq_length, max_allow_length)
                )
